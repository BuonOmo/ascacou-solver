...
fn main() {
	...
	if args.sequence {
		println!(
			"{}",
			Solver::best_continuation(args.board, Duration::from_secs(args.duration))
				.iter()
		...
		println!(
			"{}",
			Solver::best_move(args.board, Duration::from_secs(args.duration))
				.and_then(|mov| Some(mov.to_string()))
